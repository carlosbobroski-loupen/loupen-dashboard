-- db/migrations/078_webhook_enriquece_o_lead.sql
--
-- O DEFEITO, encontrado a partir de um caso concreto que o usuario apontou
-- (Alexandre Mendes Pereira / AMP CONSULTORIA, 2026-09-15):
--
-- Existiam DOIS caminhos de entrada do RD Station que nao se encontravam:
--
--   WEBHOOK (tempo real)  -> contact + conversion_event (tabela antiga, SEM
--                            nenhuma coluna de UTM). 19 linhas no total.
--   PULL de descoberta    -> lead + lead_conversion_event (COM utm_source,
--   (lote, agendado)         utm_medium, utm_campaign, utm_content, utm_term,
--                            utm_id, midia, cargo, tamanho). 767 linhas.
--
-- Consequencia pratica: o lead que chega AGORA aparece no dashboard como uma
-- linha crua, so com a origem. O que chegou no ultimo lote aparece completo.
--
-- O caso do Alexandre mostra o tamanho do desperdicio. O RD entregou, no
-- webhook, TODO o rastro da campanha paga:
--
--   utm_source=facebook  utm_medium=paid_social  utm_content=V5_AMARELO
--   utm_campaign=LOGMEIN_RESCUE-LICENCA-ECO_09_26_
--   utm_id=120256183649680492   <- o ID DO ANUNCIO no Meta
--   utm_term=[CJ07][META][[INTERESSE][BRASIL][v3]
--   Midia=Facebook  Cargo=analista  Tamanho=1 a 19 funcionarios
--   tags=[rescue-eco-2026, facebook-lead-ads]
--
-- Nada disso chegava na tela. O payload ficava parado no staging.
--
-- A CORRECAO: o ramo ConversionEvent passa a alimentar tambem `lead` e
-- `lead_conversion_event`, e a classificar o lead na hora.
--
-- POR QUE O MAPEAMENTO NAO PODE SER COPIADO do ramo LeadConversion: o payload
-- do webhook e ANINHADO e cru (formato da API do RD); o do pull vem ACHATADO
-- pelo coletor no n8n. Sao as mesmas informacoes em caminhos diferentes do
-- JSON. Copiar o mapeamento do outro ramo produziria NULL em tudo -- e a
-- reconciliacao diria 'ok', porque conta linhas e nao campos preenchidos.
--
-- O QUE NAO MUDA: o ramo continua escrevendo em contact e conversion_event.
-- Nada foi removido -- outras partes podem depender disso, e remover sem
-- medir seria trocar um defeito por outro.
...
BEGIN
  ...
  IF v_rows_source > 0 THEN
    INSERT INTO opportunity (source_system, source_id, account_id, record_type_name, stage_name, amount, mrr, duracao_meses_contrato, owner_id, created_at_source, closed_at_source, collected_at)
    SELECT
      'salesforce', s.payload->>'Id', a.id, s.payload#>>'{RecordType,Name}', s.payload->>'StageName',
      NULLIF(s.payload->>'Amount','')::numeric, NULLIF(s.payload->>'MRR__c','')::numeric,
    ...
    ON CONFLICT (source_system, source_id) DO UPDATE SET
      ...
      amount = EXCLUDED.amount, mrr = EXCLUDED.mrr, duracao_meses_contrato = EXCLUDED.duracao_meses_contrato, owner_id = EXCLUDED.owner_id,
