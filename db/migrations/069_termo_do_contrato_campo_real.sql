-- db/migrations/069_termo_do_contrato_campo_real.sql
-- Subtask 2.16 (pre-requisito). Corrige um campo INVENTADO no ramo Opportunity
-- de fn_run_ingest_batch.
--
-- O DEFEITO: a funcao lia `DuracaoMesesContrato__c`. Esse campo NAO EXISTE na
-- org da Loupen -- confirmado pelo describe de Opportunity em 2026-09-14, que
-- lista 357 campos e nenhum com esse nome. Foi escrito contra seed curado a
-- mao, nunca contra a fonte real.
--
-- O campo real e `Termo_do_Contrato__c` (double, label "Termo do Contrato"),
-- preenchido em 26.194 das 92.582 oportunidades.
--
-- POR QUE ISSO IMPORTA: `duracao_meses_contrato` sustenta o valor de contrato
-- (MRR x meses, calculado so quando o negocio e Ganho). Com o nome errado,
-- TODA oportunidade entraria com duracao NULL -- e a reconciliacao reportaria
-- `status: ok`, porque ela conta LINHAS, nao campos preenchidos. Perda
-- silenciosa: o pior tipo.
--
-- Provado por teste negativo antes da correcao (em transacao com ROLLBACK):
-- payload com "Termo_do_Contrato__c":"12.0" -> duracao_meses_contrato = NULL,
-- status da ingestao 'ok'. Depois desta migration, o mesmo payload grava 12.
--
-- O CAST: o valor vem como double do Salesforce ("12.0", "1.0"). Em Postgres
-- '12.0'::integer FALHA. Por isso passa por numeric e round() antes de integer,
-- em vez de castar direto -- um cast direto so quebraria em producao, no
-- primeiro registro real.
--
-- LICAO (mesma familia dos 4 filtros inventados do contrato e do `ilegivel`):
-- nome de campo de fonte externa e DADO, nao suposicao. Tem de ser lido do
-- describe da fonte antes de entrar em codigo.
...
BEGIN
  ...
  IF v_rows_source > 0 THEN
    ...
    SELECT
      ...
      NULLIF(s.payload->>'Amount','')::numeric, round(NULLIF(s.payload->>'Termo_do_Contrato__c','')::numeric)::integer,
