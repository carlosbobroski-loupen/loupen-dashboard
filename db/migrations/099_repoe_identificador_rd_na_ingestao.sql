-- db/migrations/099_repoe_identificador_rd_na_ingestao.sql
--
-- Repõe `lead.identificador_rd` na ingestão. A coluna e o índice (migration 081)
-- nunca saíram do banco; o que sumiu foi a GRAVAÇÃO, dentro de
-- `fn_run_ingest_batch`.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- O MODO DE FALHA — leia isto antes de reescrever qualquer função
-- ─────────────────────────────────────────────────────────────────────────────
...
--
-- Foi o que aconteceu: a 090 foi escrita sobre o corpo da 073 -- que era o topo
-- do `schema_migrations` naquele momento, porque o ledger estava dessincronizado
-- e parado na 073 (a 074, LGPD não aprovada, abortava o runner; ver
-- db/propostas/README.md). Só que o BANCO já estava na 081. Ao aplicar a 090,
-- `CREATE OR REPLACE FUNCTION` substituiu o corpo inteiro e levou junto o que a
-- 081 tinha acrescentado. Nenhum erro, nenhum aviso: a função continuou
-- ingerindo tudo, só parou de gravar um campo.
--
-- A DEFESA, sempre:
--   1. partir de `pg_get_functiondef(...)` -- o corpo VIVO, não o último arquivo;
--   2. diffar o corpo novo contra o vivo e exigir ADIÇÃO PURA;
--   3. provar por COMPORTAMENTO, não por leitura ("a string está lá" é satisfeito
--      por um comentário).
--
-- Esta migration seguiu os três. O corpo abaixo é `pg_get_functiondef` do corpo
-- vivo (que já contém a moeda da 090 e o estágio da 092) com exatamente três
-- inserções de texto -- nenhum caractere removido, verificado por reversibilidade
-- byte a byte. NÃO reaplique o arquivo da 081: ele descende do corpo antigo e
-- jogaria fora a 090 e a 092, repetindo o erro que esta migration conserta.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- O QUE VOLTA
-- ─────────────────────────────────────────────────────────────────────────────
-- No bloco `Lead` do Salesforce, três trechos da 081:
--   * `identificador_rd` na lista de colunas do INSERT;
--   * `NULLIF(s.payload->>'Identificador_RD__c','')` na lista de valores;
--   * `identificador_rd = COALESCE(EXCLUDED.identificador_rd, lead.identificador_rd)`
--     no ON CONFLICT -- o campo SÓ AVANÇA: NULL da fonte não apaga o que já foi
--     capturado.
--
-- MEDIDO EM 2026-09-16, antes de aplicar:
--   * 313 leads distintos do Salesforce chegam com `Identificador_RD__c` no
--     staging; 308 já têm o campo gravado (pela versão 081, antes da reversão) e
--     5 estão elegíveis e hoje ficam NULL. É esse o tamanho do rombo.
--
-- O QUE ISTO **NÃO** É: `Identificador_RD__c` não é chave de junção. Os 313
-- valores da janela são todos o MESMO número (`1160539`) -- ele identifica a
-- conta ou a integração, não a pessoa. A 081 apostava nele como "chave canônica
-- RD<->SF"; a medição de 2026-09-15 derrubou essa hipótese. Ele é ingerido como
-- ATRIBUTO de origem, para o dia em que alguém do RD explicar o que significa.
-- Nenhuma junção depende disto hoje, e nenhuma deve passar a depender sem nova
-- medição. Repõe-se mesmo assim porque reversão silenciosa não pode ficar de pé.
--
-- ASSERÇÃO: db/queries/verificacao/identificador-rd-na-ingestao.sql -- ingere um
-- Lead de teste pelo caminho real dentro de BEGIN/ROLLBACK e exige a coluna
-- preenchida. Rodada contra o corpo anterior a esta migration, ela FALHA (foi
-- assim que a regressão foi reproduzida).
--
-- ROLLBACK: não há down migration (db/README.md). Para desfazer, uma migration
-- nova que reemita a função a partir do corpo vivo REMOVENDO os três trechos --
-- e não voltando ao arquivo da 092, pelo motivo explicado acima.
