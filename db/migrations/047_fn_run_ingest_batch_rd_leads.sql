-- db/migrations/047_fn_run_ingest_batch_rd_leads.sql
-- Subtask 8.4 — v8 de fn_run_ingest_batch. Fecha a lacuna que a auditoria de
-- 2026-09-09 encontrou: NENHUM fluxo lia a base de leads do RD Station para
-- dentro do banco. O fluxo agendado existente (3l8hobRXtjbzodZc, 20 nós)
-- parte de contatos JÁ vinculados no Postgres para buscar funil e workflow —
-- a descoberta de lead era um pressuposto que ninguém implementou. O sintoma
-- foi `lead` com 13 registros, todos `source_system = 'seed_test'`.
--
-- O QUE ESTA MIGRATION ACRESCENTA
--   RDLead          descoberta de lead + contato a partir da base do RD
--   LeadConversion  eventos de conversão com firmografia e atribuição
--                   do INSTANTE da conversão (lead_conversion_event, 045)
--
-- O QUE ELA CORRIGE NO QUE JÁ EXISTIA
-- As seções WorkflowEvent e FunnelStage resolviam lead APENAS via
-- identity_edge → lead do Salesforce. Isso era correto enquanto o Salesforce
-- era a única porta de entrada de lead. Com leads nativos do RD passando a
-- existir, aquelas seções os deixariam órfãos de funil e de workflow. Agora
-- as duas usam fn_resolve_lead_por_rd_uuid (migration 046), que tenta o lead
-- do Salesforce primeiro (canônico, é quem carrega oportunidade e receita) e
-- cai no lead nativo do RD só quando não há vínculo.
--
-- Efeito colateral necessário: fn_record_touchpoint não pode mais receber
-- 'salesforce' fixo como fonte do lead — recebe o source_system real do lead
-- resolvido. Sem isso, touchpoint de lead nativo do RD nunca casaria.
--
-- POR QUE cargo_grupo E tamanho_empresa NÃO SÃO MATERIALIZADOS
-- São funções puras sobre tabela de regra (fn_cargo_grupo,
-- fn_normalizar_tamanho_empresa). Materializá-los no lead ficaria obsoleto no
-- instante em que alguém acrescentasse uma regra de cargo — e acrescentar
-- regra é exatamente o que o contrato §1.3 manda fazer conforme a lista de
-- distintos aparece. Ficam derivados na view (migration 048).
--
-- Já `cargo` e `tamanho_empresa_bruto` no lead SÃO armazenados, e não é
-- redundância: são o valor ATUAL DO CADASTRO, fato distinto do valor no
-- momento da conversão que vive em lead_conversion_event. Verificado nos
-- dados reais — o mesmo contato tem `Diretor` num evento e
-- `CEO, Founder, Sócio` noutro. Os dois fatos são verdadeiros.

-- ---------------------------------------------------------------------------
-- Colunas de perfil no lead. Todas nullable: nenhum lead existente muda de
-- estado com esta migration.
-- ---------------------------------------------------------------------------
ALTER TABLE lead ADD COLUMN IF NOT EXISTS cargo                  text;
ALTER TABLE lead ADD COLUMN IF NOT EXISTS tamanho_empresa_bruto  text;
ALTER TABLE lead ADD COLUMN IF NOT EXISTS atendido_por           text;
ALTER TABLE lead ADD COLUMN IF NOT EXISTS tags                   text[];
ALTER TABLE lead ADD COLUMN IF NOT EXISTS teste_regra            text;

COMMENT ON COLUMN lead.cargo IS
  'job_title do cadastro do RD Station (cobertura medida: 90,0%). Valor ATUAL — o valor no instante de cada conversão vive em lead_conversion_event.cargo. cargo_grupo é derivado na view, nunca materializado aqui.';
COMMENT ON COLUMN lead.tamanho_empresa_bruto IS
  'cf_tamanho_da_empresa cru (cobertura 36,4%), texto livre com escalas conflitantes. A faixa canônica sai de fn_normalizar_tamanho_empresa na view — data-contract.md §1.4.';
COMMENT ON COLUMN lead.atendido_por IS
  'cf_falou_com (cobertura 82,0%) — quem atendeu o lead. Dimensão não prevista no contrato original; entrou por medição.';
COMMENT ON COLUMN lead.tags IS
  'tags do RD Station (cobertura 94,4%, a maior da base depois de identidade) e o único campo nativamente multivalor da fonte. Filtro por interseção de array.';
COMMENT ON COLUMN lead.teste_regra IS
  'Nome da regra de lead_teste_regra que marcou este lead como teste. Guardado para que a aba Qualidade de dados mostre a contagem POR REGRA — data-contract.md §1.6 proíbe exclusão silenciosa.';

CREATE INDEX IF NOT EXISTS ix_lead_tags        ON lead USING gin (tags);
CREATE INDEX IF NOT EXISTS ix_lead_cargo       ON lead (cargo)        WHERE cargo IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_lead_atendido_por ON lead (atendido_por) WHERE atendido_por IS NOT NULL;

-- ---------------------------------------------------------------------------
-- fn_reclassificar_lead — extrai o bloco de versionamento de classificação
-- de origem para uma função reutilizável.
--
-- NOTA DE ESCOPO DELIBERADA: a seção Lead (Salesforce) mantém o bloco inline
-- que já tinha. Não migrei aquele caminho para esta função porque não existe
-- dado real de Salesforce no banco para verificar a equivalência por
-- execução (a ingestão do Salesforce está bloqueada no consentimento OAuth),
-- e trocar um caminho que funciona por outro não testável seria risco sem
-- retorno. Quando o Salesforce entrar, unificar os dois.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION fn_reclassificar_lead(p_lead_id bigint)
RETURNS boolean
LANGUAGE plpgsql
AS $fn$
DECLARE
...
BEGIN
  ...
  -- Nada mudou: não versiona. Evita uma versão nova por execução de pull.
  IF v_versao_atual.id IS NOT NULL
     AND v_versao_atual.valor_bruto IS NOT DISTINCT FROM v_result.valor_bruto THEN
    RETURN false;
  END IF;
...
COMMENT ON FUNCTION fn_reclassificar_lead(bigint) IS
  'Versiona a classificação de origem de um lead (AC-5.2). Devolve true quando gravou versão nova, false quando o sinal bruto não mudou — o retorno é o contador de idempotência, e por isso NUNCA deve ser chamada com PERFORM (erro já cometido e corrigido na migration 042).';
...
BEGIN
  ...
  IF v_rows_source > 0 THEN
    ...
    INSERT INTO ingest_run (source, object, started_at, finished_at, rows_source, rows_target, status)
    VALUES ('rd_station', 'RDLead', now(), now(), v_rows_source, v_rows_target, CASE WHEN v_rows_source = v_rows_target THEN 'ok' ELSE 'partial' END);
  ...
  IF v_rows_source > 0 THEN
    ...
    LOOP
      ...
      ON CONFLICT (lead_id, origem_conversao, occurred_at) DO NOTHING;

      -- FOUND é false quando o ON CONFLICT descartou. É o contador de
      -- idempotência correto — e o motivo de este bloco usar INSERT direto
      -- e não PERFORM (erro cometido e corrigido na migration 042).
      IF FOUND THEN
    ...
    VALUES ('rd_station', 'LeadConversion', now(), now(), v_rows_source, v_conv_gravadas,
    ...
    IF v_leads_escalados > 0 THEN
      INSERT INTO ingest_run (source, object, started_at, finished_at, rows_source, rows_target, status, error_message)
      VALUES ('rd_station', 'LeadConversion_MarcacaoTeste', now(), now(), v_leads_escalados, v_leads_escalados, 'ok',
  ...
  IF v_rows_source > 0 THEN
    ...
    FOR r_we IN
      ...
      WHERE s.batch_id = p_batch_id AND s.object_type = 'WorkflowEvent'
    LOOP
      -- v8: resolve via fn_resolve_lead_por_rd_uuid (Tier1 Salesforce, senao
  ...
  IF v_rows_source > 0 THEN
    ...
    GET DIAGNOSTICS v_rows_target = ROW_COUNT;
    INSERT INTO ingest_run (source, object, started_at, finished_at, rows_source, rows_target, status)
    VALUES ('ads', 'ad_campaign', now(), now(), v_rows_source, v_rows_target, CASE WHEN v_rows_source = v_rows_target THEN 'ok' ELSE 'partial' END);
...
COMMENT ON FUNCTION fn_run_ingest_batch(text) IS
  'v8 (migration 047): acrescenta RDLead (descoberta de lead do RD Station — a lacuna que deixava `lead` com só 13 linhas de seed_test) e LeadConversion (evento de conversão com firmografia e atribuição do instante, em lead_conversion_event). Corrige WorkflowEvent/FunnelStage, que resolviam lead apenas via Salesforce e deixariam órfãos os leads nativos do RD — agora usam fn_resolve_lead_por_rd_uuid. Demais seções inalteradas desde v7/043.';
