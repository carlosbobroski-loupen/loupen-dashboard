-- db/migrations/101_repoe_enriquecimento_do_webhook_rd.sql
--
-- Repõe o enriquecimento do lead pelo WEBHOOK do RD Station — a segunda metade
-- do que a 090 reverteu. A primeira (`identificador_rd`, da 081) voltou na 099.
-- Esta é a que dói: é a atribuição de mídia paga.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- O MESMO MODO DE FALHA DA 099
-- ─────────────────────────────────────────────────────────────────────────────
-- REESCREVER UMA FUNÇÃO INTEIRA A PARTIR DE UM ARQUIVO DE MIGRATION ANTIGO
-- REVERTE, EM SILÊNCIO, TUDO QUE VEIO DEPOIS DAQUELE ARQUIVO.
--
-- A 090 foi escrita sobre o corpo da 073 -- o topo do `schema_migrations` naquele
-- momento, porque o ledger estava dessincronizado e parado ali (a 074, LGPD não
-- aprovada, abortava o runner; ver db/propostas/README.md). O banco já estava na
-- 081. `CREATE OR REPLACE FUNCTION` trocou o corpo inteiro e levou junto os
-- blocos da 078 e da 081. Sem erro, sem aviso.
--
-- ARMADILHA DE MEDIÇÃO, registrada porque custou uma rodada: procurar
-- `last_conversion` no corpo vivo dá FALSO POSITIVO. Ele aparece 9 vezes mesmo
-- sem a 078 -- 9 é a base da 073. A 078 levava para 39 e trazia `custom_fields`
-- junto. O token que decide é `custom_fields` (ou `Campanha de Origem`), que
-- estava em ZERO.
--
-- MÉTODO (idêntico ao da 099): corpo de `pg_get_functiondef` relido no momento de
-- gerar este arquivo, com UMA inserção de texto. Verificado por reversibilidade:
-- desfazer exatamente o trecho inserido devolve o corpo vivo byte a byte.
-- Diff: 123 linhas adicionadas, ZERO removidas, zero modificadas.
-- NÃO reaplique o arquivo da 078: ele descende do corpo da 073 e jogaria fora
-- moeda (090), estágio (092), vínculo de oportunidade e o `identificador_rd` que
-- a 099 acabou de repor.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- O QUE VOLTA
-- ─────────────────────────────────────────────────────────────────────────────
-- No ramo `ConversionEvent` (o caminho do webhook, tempo real), o bloco da 078:
--   * `INSERT INTO lead` — antes disto o webhook escrevia SÓ em `contact` e na
--     tabela antiga `conversion_event`, que não tem coluna de UTM. O lead nem
--     existia como linha em `lead`.
--   * `INSERT INTO lead_conversion_event` — com `utm_source`, `utm_medium`,
--     `utm_campaign`, `utm_content`, `utm_term`, `utm_id` (o ID do anúncio no
--     Meta), `gclid`, `Mídia` e `Campanha de Origem`, lidos com COALESCE entre
--     `custom_fields` e `last_conversion.content`, porque a fonte popula os dois
--     e nem sempre ambos.
--   * `PERFORM fn_reclassificar_lead` — para o lead recém-chegado não ficar
--     'NaoAtribuido' até a próxima reclassificação manual.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- TAMANHO DO ROMBO, MEDIDO EM 2026-09-16 ANTES DE CONSERTAR
-- ─────────────────────────────────────────────────────────────────────────────
-- A perda é PROSPECTIVA, não histórica -- e por sorte de calendário, não por
-- desenho:
--   * eventos de webhook (`stg_rdstation`, object_type='ConversionEvent') em toda
--     a base: 24. TODOS os 24 trazem `custom_fields` com `utm_source` e `utm_id`
--     (cobertura 100%: quando o webhook dispara, ele sempre traz UTM).
--   * os 24 já viraram linha em `lead` E já têm evento com `utm_source`
--     preenchido. Nenhum foi perdido.
--   * eventos que chegaram DEPOIS da reversão (090 aplicada 2026-09-16 11:43Z):
--     ZERO. O último webhook entrou às 04:41Z do mesmo dia, ainda com a 078 viva.
--
-- Ou seja: NÃO HÁ NADA PARA REPROCESSAR. A janela entre a reversão e este
-- conserto não teve tráfego de webhook. O próximo evento, porém, perderia tudo --
-- não parte: o lead sequer viraria linha em `lead`.
--
-- ASSERÇÃO: db/queries/verificacao/webhook-rd-enriquece-o-lead.sql -- ingere um
-- payload de webhook com UTM em `custom_fields` pelo caminho real, dentro de
-- BEGIN/ROLLBACK, e exige lead + evento com UTM. Rodada contra o corpo anterior a
-- esta migration, FALHA (foi assim que a regressão foi reproduzida).
--
-- ROLLBACK: não há down migration (db/README.md). Para desfazer, migration nova
-- reemitida do corpo vivo REMOVENDO o bloco -- nunca voltando a um arquivo antigo.
