-- db/migrations/073_mrr_separado_de_amount.sql
-- Separa MRR de Amount, porque a premissa documentada na migration 012 esta
-- ERRADA contra a org real.
--
-- O QUE A 012 AFIRMAVA, em comentario: "Amount e MRR bruto, NUNCA valor de
-- contrato". Medido na org em 2026-09-14, linha real de Opportunity:
--
--     Amount = 708,00   |   MRR__c = 2367,05   |   RecordType = New Freshworks
--
-- Sao campos diferentes com valores diferentes. Guardar `Amount` numa coluna
-- cujo COMMENT afirma ser MRR faria qualquer metrica de receita nascer errada
-- -- e errada de um jeito que ninguem percebe, porque o numero existe e parece
-- plausivel.
--
-- DECISAO: guardar OS DOIS, crus, cada um com o nome do campo de onde veio.
-- `amount` = Opportunity.Amount. `mrr` = Opportunity.MRR__c. Nenhum dos dois e
-- rebatizado, nenhum e derivado do outro. Qual deles sustenta receita por
-- RecordType e uma decisao de negocio que ainda nao foi tomada -- e que agora
-- PODE ser tomada com os dois numeros na mesa, em vez de ser embutida numa
-- suposicao de 2026-08.
--
-- COBERTURA MEDIDA: MRR__c preenchido em 9.321 das 92.582 oportunidades (10%).
-- Cobertura baixa e um fato da fonte, nao um motivo para substituir por Amount.

ALTER TABLE opportunity ADD COLUMN IF NOT EXISTS mrr numeric;

COMMENT ON COLUMN opportunity.mrr IS
  'Opportunity.MRR__c cru, do Salesforce. Preenchido em ~10% das oportunidades. NAO e derivado de amount nem substituivel por ele -- sao campos distintos na fonte (ver migration 073).';

COMMENT ON COLUMN opportunity.amount IS
  'Opportunity.Amount cru, do Salesforce. ATENCAO: nao e o MRR, apesar do que a migration 012 afirmava -- ha linhas reais com Amount=708,00 e MRR__c=2367,05. Para MRR use a coluna mrr (migration 073).';
