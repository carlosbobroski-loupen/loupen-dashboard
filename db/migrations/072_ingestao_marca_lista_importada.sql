-- db/migrations/072_ingestao_marca_lista_importada.sql
-- Liga a regra de lista importada (migration 071) na ingestao de leads do RD
-- Station, para que lista nova seja marcada sozinha em vez de depender de
-- alguem lembrar de rodar um backfill.
--
-- Sem isto, a 071 corrigiria apenas os 318 leads que ja estao na base, e a
-- proxima lista de evento entraria silenciosamente no denominador das taxas --
-- que e exatamente o defeito que a 071 existe para fechar.
--
-- DIFERENCA DELIBERADA EM RELACAO A is_teste: `is_teste` e MONOTONICO (uma
-- execucao nova nunca des-marca um lead, porque "isto e teste" e um julgamento
-- nosso). `lista_regra` e RECALCULADO a cada execucao, porque a tag e dado da
-- fonte: se a fonte tirar a tag, o lead deixa de ser de lista. Congelar seria
-- decidir contra o dado.
