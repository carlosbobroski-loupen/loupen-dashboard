-- db/migrations/043_fn_run_ingest_batch_ads.sql
-- Subtask 2.18 (ADAPT do workflow de ads existente). Liga stg_ads.object_type=
-- 'campaign' (Meta/Google/LinkedIn, já reais e testados ao vivo no workflow n8n
-- "Loupen Dashboard - Ads Data API") a `ad_campaign` (migration 003, DM-6).
-- campaign_ref_id fica NULL por desenho — nenhum match campanha↔ad_campaign é
-- forçado nesta versão (DM-6 já documenta NULL como estado válido). Owner/
-- Account/Lead/Opportunity/ConversionEvent/Lead_Atribuicao/WorkflowEvent/
-- FunnelStage (v6/042) inalterados.
...
COMMENT ON FUNCTION fn_run_ingest_batch(text) IS
  'v7 (migration 043): adiciona ad_campaign (Meta/Google/LinkedIn Ads, 2.18) ao mapeamento — campaign_ref_id NULL por desenho, sem match forçado. Demais seções inalteradas desde v6/042.';
