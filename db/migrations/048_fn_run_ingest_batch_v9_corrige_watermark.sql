-- db/migrations/048_fn_run_ingest_batch_v9_corrige_watermark.sql
-- Subtask 8.5 — CORREÇÃO DE REGRESSÃO que eu mesmo introduzi na 047.
--
-- O QUE ACONTECEU
-- Montei a 047 reaproveitando o corpo da migration 043 (v7). Mas a 044 já
-- era a v8 e havia trocado TODOS os `INSERT INTO ingest_run` espalhados por
-- `PERFORM fn_registrar_ingest(...)`, que grava `ingest_run` E `sync_state`
-- na mesma chamada. Reconstruir a partir da v7 reverteu isso: a 047 voltou a
-- gravar só `ingest_run`, e o watermark incremental (P-ING-2) pararia de
-- avançar — em SILÊNCIO, que é o pior modo de falhar. Nenhum erro, nenhum
-- log, só a ingestão incremental deixando de saber onde parou.
--
-- COMO FOI PEGO
-- Não por teste: por checar, antes de rodar qualquer coisa, se alguma
-- migration posterior à que eu usei como base mexia nas mesmas linhas. A
-- lição é a mesma de outras vezes neste projeto: reaproveitar corpo de
-- função por substituição de texto exige verificar contra a versão MAIS
-- RECENTE, não contra a que estava aberta.
--
-- ESTA MIGRATION
-- Reconstrói fn_run_ingest_batch a partir do corpo da 044 (v8 real),
-- aplicando as mesmas 4 mudanças da 047 e as duas seções novas, agora com
-- fn_registrar_ingest em todas elas. Verificado por contagem antes de
-- escrever: 8 chamadas herdadas + 3 novas, e o único `INSERT INTO
-- ingest_run` direto que resta é o catch-all `nao_mapeado`, que é
-- status='failed' de propósito (watermark não deve avançar em falha).
--
-- As colunas de perfil no lead, fn_reclassificar_lead, fn_lead_e_teste e
-- fn_resolve_lead_por_rd_uuid já vieram nas migrations 045/046/047 e não são
-- recriadas aqui.
...
COMMENT ON FUNCTION fn_run_ingest_batch(text) IS
  'v9 (migration 048): corrige a regressão da 047, que ao reaproveitar o corpo da v7 havia revertido a gravação de sync_state introduzida pela 044 — o watermark incremental pararia de avançar em silêncio. Mantém as seções novas RDLead (descoberta de lead do RD Station) e LeadConversion (evento com firmografia e atribuição do instante), agora registrando via fn_registrar_ingest. WorkflowEvent/FunnelStage resolvem lead por fn_resolve_lead_por_rd_uuid (Salesforce canônico, senão lead nativo do RD).';
