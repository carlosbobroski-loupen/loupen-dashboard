-- db/migrations/062_reconciliacao_leadconversion.sql
-- Subtask 8.19 — corrige uma metrica minha que a assercao AC-8.1 acusou.
--
-- ── O QUE A ASSERCAO PEGOU ────────────────────────────────────────────────
-- `reconciliacao-ac81.sql` verifica o invariante: execucao com `status=ok`
-- nao pode ter `rows_source != rows_target` — isso e "sincronizacao parcial
-- disfarcada de completa", o defeito EC-6.
--
-- Ela falhou em 5 execucoes minhas de `LeadConversion`, ex.: 777 -> 632, ok.
--
-- ── POR QUE ESTAVA ERRADO, E O QUE ESTAVA CERTO ───────────────────────────
-- O dado estava certo: 145 daquelas 777 linhas eram DUPLICATAS DE FONTE
-- (mesmo lead, mesma origem, mesmo instante ao segundo), colapsadas pela
-- regra de negocio do contrato 1.7. Nada foi perdido.
--
-- Errada estava a METRICA. `rows_source` contava LINHAS DE STAGING, e por
-- isso a deduplicacao legitima aparecia como perda de dado.
--
-- ── O PROBLEMA MAIS FUNDO QUE ISSO EXPOS ──────────────────────────────────
-- `rows_target` contava os eventos RECEM-INSERIDOS. Numa reexecucao
-- idempotente — o comportamento correto e desejado — nada e inserido, logo
-- daria 0, e um lote perfeito seria acusado de perda TOTAL. O invariante
-- AC-8.1 e a idempotencia eram incompativeis por construcao.
--
-- ── A CORRECAO ────────────────────────────────────────────────────────────
--   rows_source  eventos DISTINTOS no lote (por lead+origem+instante)
--   rows_target  quantos deles ESTAO no destino depois desta execucao
--
-- Assim o invariante volta a significar o que devia: "tudo que o lote trazia
-- chegou ao destino". Orfaos (contato sem lead resolvido) continuam fazendo
-- os dois numeros divergirem e o status virar `partial` — que e uma perda
-- real e deve aparecer.
--
-- "Quantos entraram AGORA" continua sendo util, e segue reportado — como
-- objeto separado (`LeadConversion_Novos`), nao somado. Metrica de operacao e
-- metrica de reconciliacao respondem perguntas diferentes.
--
-- ── TERCEIRA VEZ QUE O CONTADOR ME ENGANA ─────────────────────────────────
-- Migration 042: `PERFORM` descartava o retorno que servia de contador.
-- Migration 058: `FOUND` passou a ser true tambem em preenchimento de coluna.
-- Agora: `rows_source`/`rows_target` medindo a coisa errada.
-- Nos tres casos o DADO estava correto e a METRICA mentia. Metrica que mente
-- e pior que metrica ausente, porque ninguem desconfia dela — e as tres so
-- apareceram porque havia assercao executavel olhando.
