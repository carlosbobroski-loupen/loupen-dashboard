-- db/migrations/060_atribuicao_forma_plana.sql
-- Subtask 8.17 — corrige um erro de LEITURA DA FONTE que a coluna
-- atribuicao_status (057) revelou. E o melhor exemplo, nesta epic, de
-- instrumentacao pagando o proprio custo.
--
-- ── O QUE A COLUNA REVELOU ────────────────────────────────────────────────
-- Depois da reingestao: 434 eventos `ok`, 270 `ausente` e **61 `ilegivel`**.
-- Fui olhar os 61 e eles eram os MELHOR atribuidos da base: 47 com
-- utm_campaign, 45 com midia. Nao faziam sentido como "falha de leitura".
--
-- Consultando a API para um deles, a causa: o RD Station usa DUAS FORMAS
-- para `traffic_source`, e o coletor conhecia so uma.
--
--   A) `encoded_<base64>` — blob de sessao com first_session/current_session.
--      Atribuicao de primeiro e ultimo toque. Conversoes de site.
--
--   B) uma STRING SIMPLES, ex.: "facebook", acompanhada de `traffic_medium`
--      ("paid_social"), `traffic_campaign` e `traffic_value`. Conversoes de
--      Facebook Lead Ads.
--
-- O coletor tentava decodificar B como base64, falhava, e marcava `ilegivel`
-- — descartando a atribuicao de MIDIA PAGA, que e exatamente a que sustenta
-- calculo de ROI. Dos 61, **14 ficaram sem nenhuma atribuicao capturada**.
--
-- ── POR QUE ISTO E UM ACHADO E NAO SO UM BUG ──────────────────────────────
-- Eu havia relatado a cobertura do eixo B (campanha de midia paga) como 7,6%,
-- medida sobre os campos `cf_utm_*` do cadastro. A forma plana e uma fonte
-- ADICIONAL de atribuicao paga que estava sendo jogada no lixo pelo proprio
-- coletor. A cobertura real do eixo B e maior do que eu reportei.
--
-- A forma B nao tem historico de sessao, entao primeiro/ultimo toque
-- continuam null nesses eventos — nao se inventa uma sessao a partir dela.
-- Status novo: `ok_plano`, para que a diferenca entre as duas formas fique
-- contavel em vez de escondida atras de um `ok` generico.
--
-- ── COMO O ERRO ANTERIOR TINHA A MESMA ASSINATURA ─────────────────────────
-- A versao anterior misturava "campo ausente" com "falha de decodificacao"
-- num `null` so. Corrigi isso na 057 e, na mesma correcao, criei uma versao
-- menor do mesmo erro: passei a chamar `ilegivel` tanto uma falha real quanto
-- um formato que eu simplesmente nao conhecia. A licao que fica e sobre o
-- nome: `ilegivel` afirma que o dado esta errado, quando o que estava errado
-- era o leitor.
...
COMMENT ON FUNCTION fn_run_ingest_batch(text) IS
  'v12 (migration 060): o ON CONFLICT de lead_conversion_event passa a (a) preencher colunas de atribuicao quando NULL e (b) substituir o status quando ele e `ilegivel` — que registra falha NOSSA de leitura, nao fato sobre o lead, e portanto pode ser corrigido quando o coletor melhora. Nenhum dado de negocio e sobrescrito e a dedup segue intacta. Resto herdado da v11/058.';
