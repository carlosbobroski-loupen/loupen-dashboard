-- db/migrations/061_qualidade_atribuicao_generica.sql
-- Subtask 8.18 — o bloco de atribuição de `fn_qualidade_dados` passa a agrupar
-- por status DINAMICAMENTE, em vez de listar os valores um por um.
--
-- POR QUE: a versão da 059 tinha um `count(*) FILTER (WHERE status = 'ok')`
-- por valor conhecido. Bastou a 060 acrescentar `ok_plano` para que a aba
-- deixasse de contá-lo — o valor existia no banco e não aparecia na tela.
-- Enumerar valores no lugar de agrupar é a mesma família de erro que produziu
-- os 4 filtros inventados do contrato: código decidindo quais valores
-- existem, em vez de perguntar ao dado.
--
-- Agora, status novo aparece sozinho. A interface tem rótulo para os que
-- conhece e mostra o valor cru para os que não conhece — visível e feio, que
-- é melhor que invisível.
...
COMMENT ON FUNCTION fn_qualidade_dados() IS
  'Cumpre data-contract.md §1.6 (exclusão por regra) e §1.3 (valores sem regra). Desde a migration 061 o bloco de atribuição AGRUPA por status em vez de enumerar valores conhecidos — bastou a 060 criar `ok_plano` para a versão anterior deixar de contá-lo, e enumerar valores no código é a mesma família de erro que gerou os 4 filtros inventados do contrato. Expõe também `sem_nenhum_sinal`: eventos sem sessão, sem utm e sem mídia, que é o número honesto de "não sabemos de onde veio".';
