-- db/migrations/081_identificador_rd_na_ingestao.sql
--
-- Traz `Identificador_RD__c` do Salesforce para a base.
--
-- POR QUE IMPORTA: e a chave CANONICA entre RD Station e Salesforce -- o proprio
-- RD grava o uuid do contato nesse campo do lead do CRM. Hoje o cruzamento das
-- duas pontas e feito por E-MAIL, que e Tier 1 mas e mais fraco: e-mail muda,
-- pessoa usa dois, e-mail corporativo vira pessoal.
--
-- O CAMPO FOI MEDIDO em 2026-09-14 (423 leads preenchidos, todos a partir de
-- 06/2026), registrado na memoria do projeto como "a chave canonica, melhor que
-- casar por e-mail" -- e a carga de 12 meses foi feita SEM ele no SOQL. O
-- workflow recorrente ja o traz desde 2026-09-15; esta migration cria o destino.
--
-- COBERTURA ESPERADA E BAIXA DE PROPOSITO: 423 de 7.076. Nao substitui o e-mail,
-- COMPLEMENTA. Onde existe, e mais confiavel; onde nao existe, o e-mail continua
-- valendo. Isso fica explicito na view_pessoa (migration 082), que declara qual
-- regra resolveu cada pessoa.

ALTER TABLE lead ADD COLUMN IF NOT EXISTS identificador_rd text;

COMMENT ON COLUMN lead.identificador_rd IS
  'Salesforce Lead.Identificador_RD__c -- o uuid do contato no RD Station, gravado pelo proprio RD. Chave canonica RD<->SF, mais forte que e-mail. Cobertura baixa por natureza da fonte (so preenchido a partir de 06/2026). Ver migration 081.';

CREATE INDEX IF NOT EXISTS idx_lead_identificador_rd
  ON lead (identificador_rd) WHERE identificador_rd IS NOT NULL;
...
BEGIN
  ...
  IF v_rows_source > 0 THEN
    INSERT INTO lead (source_system, source_id, nome, empresa, email, telefone, status, origem_bruta, campanha_linkedin_bruta, identificador_rd, converted_account_id, owner_id, created_at_source, collected_at)
    SELECT
      'salesforce', s.payload->>'Id', s.payload->>'Name', s.payload->>'Company', s.payload->>'Email', s.payload->>'Phone',
      s.payload->>'Status', s.payload->>'LeadSource', s.payload->>'Campanha_LinkedIn__c',
      NULLIF(s.payload->>'Identificador_RD__c',''), s.payload->>'ConvertedAccountId',
    ...
    ON CONFLICT (source_system, source_id) DO UPDATE SET
      ...
      -- identificador_rd so avanca: NULL da fonte nao apaga o que ja foi capturado.
