-- db/migrations/057_atribuicao_status.sql
-- Subtask 8.14 — separa "a fonte não mandou" de "não conseguimos ler".
--
-- O PROBLEMA
-- O coletor decodifica `traffic_source` (base64 de um JSON com first_session
-- e current_session) e devolvia `null` em DOIS casos distintos:
--   a) o campo não veio no payload do evento
--   b) o campo veio e a decodificação falhou
--
-- Somados, os dois viraram um número só na aba de qualidade: "332 eventos sem
-- sessão decodificável". Isso esconde a única distinção que importa para
-- agir: (a) é limitação da fonte e não há o que fazer no nosso lado; (b) é
-- bug nosso e tem correção.
--
-- Eu havia apresentado a atribuição de primeiro/último toque como "de graça e
-- nativa". É nativa, mas cobre 434 de 766 eventos reais (57%) — e sem esta
-- coluna eu não conseguia dizer se o resto era ausência na fonte ou falha
-- minha de leitura.
--
-- VALORES
--   ok         decodificou; primeiro_toque_bruto/ultimo_toque_bruto válidos
--   ausente    `traffic_source` não veio no payload — limitação da fonte
--   ilegivel   veio e a decodificação falhou — investigar do nosso lado
--   NULL       evento ingerido antes desta migration (não retroagido: não
--              existe como recuperar a informação sem reingerir, e inventar
--              um valor seria pior que admitir a lacuna)

ALTER TABLE lead_conversion_event
  ADD COLUMN IF NOT EXISTS atribuicao_status text;

COMMENT ON COLUMN lead_conversion_event.atribuicao_status IS
  'ok | ausente | ilegivel — separa "a fonte não mandou traffic_source" de "mandou e não conseguimos decodificar". NULL nos eventos ingeridos antes da migration 057, deliberadamente não retroagido: sem reingerir não há como saber qual dos dois era, e preencher um palpite apagaria a lacuna em vez de mostrá-la.';

CREATE INDEX IF NOT EXISTS ix_lead_conversion_event_atrib_status
  ON lead_conversion_event (atribuicao_status)
  WHERE atribuicao_status IS NOT NULL;

-- ---------------------------------------------------------------------------
-- fn_run_ingest_batch v10 — grava a coluna nova. Só a seção LeadConversion
-- muda; todo o resto vem da v9 (migration 048) sem alteração.
--
-- Reaproveitamento por substituição verificada, e NÃO por reescrita a partir
-- de uma versão antiga: foi exatamente esse atalho que, na 047, reverteu em
-- silêncio a gravação de sync_state introduzida pela 044.
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION fn_run_ingest_batch(text) IS
  'v10 (migration 057): grava lead_conversion_event.atribuicao_status, separando "traffic_source ausente na fonte" de "presente e ilegivel" — a distincao que decide se a lacuna de atribuicao e limitacao da fonte ou bug nosso. Todo o resto herdado da v9/048 sem alteracao, por substituicao verificada e nao por reescrita a partir de versao antiga (o atalho que causou a regressao da 047).';
