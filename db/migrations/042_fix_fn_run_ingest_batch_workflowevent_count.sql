-- db/migrations/042_fix_fn_run_ingest_batch_workflowevent_count.sql
-- Bug real encontrado testando 040/041 de verdade: reexecutar o mesmo
-- batch_id (idempotência) corretamente NÃO duplicava a linha em
-- lead_touchpoint (ON CONFLICT DO NOTHING protegia o dado), mas o CONTADOR
-- reportado em ingest_run/rows_target continuava dizendo "1 gravado" na
-- segunda execução, porque o loop usava PERFORM (descarta o retorno da
-- função) em vez de checar se fn_record_touchpoint devolveu um id novo ou
-- NULL (conflito/no-op). FunnelStage já fazia a checagem de mudança ANTES
-- de inserir (correto, sem esse bug) — só WorkflowEvent tinha o problema.
-- Corrigido: SELECT ... INTO + só incrementa o contador quando o retorno
-- não é NULL. Owner/Account/Lead/Opportunity/ConversionEvent/FunnelStage
-- inalterados desta versão para a anterior.
...
COMMENT ON FUNCTION fn_run_ingest_batch(text) IS
  'v6 (migration 042): corrige contagem de WorkflowEvent em ingest_run (v5/040 tinha PERFORM, que não distinguia insert novo de no-op por ON CONFLICT) — agora só conta como gravado quando fn_record_touchpoint retorna um id novo. Demais seções inalteradas.';
